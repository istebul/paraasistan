-- Tek kullanımlık Pro erişim kodunun atomik olarak kullanılması.
-- Kod, yalnızca giriş yapmış kullanıcı tarafından ve kendi hesabı için kullanılabilir.
...
begin
  ...
  -- FOR UPDATE sayesinde aynı kodun eş zamanlı iki istekte
  -- iki kez kullanılmasını engeller.
  select *
    into v_code
  ...
  -- Kullanıcının mevcut membership kaydını kilitle.
  select *
    into v_membership
  ...
  if not found then
    ...
    -- Süreli mevcut Pro üyeliği varsa yeni kod mevcut sürenin
    -- üzerine eklenir.
    update public.memberships
    set
      status = 'active',
